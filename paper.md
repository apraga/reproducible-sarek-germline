...
and for reference pipeline.

Here, we propose to push further the FAIR (findable, accessible, interoperable
and reusable) principles by improving the reproducibility of the reference
pipeline for rare diseases, nf-core's sarek , for dependencies and database
management.
...
non-determinist output ziemann 2023.
...
reference patients than can server a ground truth @Krusch. Step 2 has been
improved with workflow manager supporting in a transparent way multiple
...
Step 1. can be partially solved using software packaging tools. Nextflow allows
for
